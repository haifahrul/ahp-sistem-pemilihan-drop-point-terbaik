...
//    $kriteriaSeleksi = $q_seleksi = "SELECT * FROM ahp_kriteria_seleksi aks LEFT JOIN ahp_kriteria ak ON ak.id_kriteria = aks.id_kriteria WHERE aks.id_seleksi = " . $seleksi;
//    $kriteriaSeleksi = querydb($kriteriaSeleksi);
...
//                $value = !empty($_POST[$valueInput]) ? $_POST[$valueInput] : 0;
//
//                $q_sql_cek_kandidat = "SELECT * FROM ahp_kandidat WHERE id_seleksi='$seleksi' AND id_kriteria='$valueIdKriteria' AND id_alternatif='$valueIdAlternatif'";
//                $q_sql_cek_kandidat = querydb($q_sql_cek_kandidat);
//                $q_sql_cek_kandidat = mysqli_fetch_row($q_sql_cek_kandidat);
//
//                if (empty($q_sql_cek_kandidat)) {
//                    $query = "INSERT INTO ahp_kandidat (id_seleksi, id_kriteria, id_alternatif, value)
//							   VALUES ('$seleksi', '$valueIdKriteria', '$valueIdAlternatif', '$value')";
//                    querydb($query);
//                } else {
//                    $query = "UPDATE ahp_kandidat SET id_seleksi='$seleksi', id_kriteria='$valueIdKriteria', id_alternatif='$valueIdAlternatif', value='$value' WHERE id_seleksi='$seleksi' AND id_kriteria='$valueIdKriteria' AND id_alternatif='$valueIdAlternatif'";
//                    querydb($query);
//                }
//            }
//        }
...
<div class="card">
    ...
    <!-- PILIH SELEKSI -->
    <form class="card-block table-border-style" method="post" action="?h=nilai-kriteria" enctype="multipart/form-data">
    ...
    <?php if (!empty($seleksi)) { /* ?>
      ...
      <tr>
      <td><?php echo $no ?></td>
      <td>Kandidat <?php echo $no ?></td>
      <td><?php echo $val['nama_alternatif'] ?></td>
      <?php
      ...
      //                                    var_dump($q_sql_value_kandidat[0]);
      //                                    exit;
